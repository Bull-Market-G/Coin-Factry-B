import CoreArrowImg from 'assets/images/core_arrow.png'
import CoreCoinImg from 'assets/images/CoreCoin.png'
...
export default function Home() {
  ...
  const tokenAddress = '0xa5677cE8aFFeD47a3ED45bb4ffcc5194717e1caf' // CORE Coin
            ...
            <AutoColumn gap="10px" justify="center" style={{ width: '100%' }}>
              ...
              >
                <AutoRow justify="center">
                  <AutoColumn>
                    <img src={CoreCoinImg} width="200px" />
                    <AutoColumn>
                  ...
                  <img src={CoreArrowImg} width="460px" height="220px" />
                </AutoRow>
                <AutoColumn>
                  <AutoRow marginBottom="10px">
                    ...
                    {sendAddress ? (
                      coinBalance ? (
                        <TYPE.main marginLeft="auto">{`Balance: ${coinBalance} CORE Coin`}</TYPE.main>
                      ) : (
                        <TYPE.main marginLeft="auto">{`Balance: 0 CORE Coin`}</TYPE.main>
                      )
                    ) : null}
                  </AutoRow>
                  <StyledInputAddress
              ...
              >
                {mintState === MintState.WALLET_CONDITION ? (
                  <TYPE.main color={theme.red1}> ● Please operate MetaMask.</TYPE.main>
                ) : null}
                {ownerCondition ? (
                  <TYPE.main color={theme.secondary1}> ● Your wallet is owner.</TYPE.main>
                ) : account ? (
                  <TYPE.main color={theme.red1}>
                    ● Your wallet is not owner. Please change correct owner wallet.
                  </TYPE.main>
                ) : (
                  <TYPE.main color={theme.red1}> ● Please connect owner wallet.</TYPE.main>
                )}
                {chainId === 80001 ? (
                  <TYPE.main color={theme.secondary1}> ● Correct chain.</TYPE.main>
                ) : chainId === undefined ? null : (
                  <TYPE.main color={theme.red1}> ● Different chain. Please change correct chain.</TYPE.main>
                )}
                {inputAmount ? null : (
                  <TYPE.main color={theme.red1}> ● Please input the number of coins you want to mint.</TYPE.main>
                )}
                {sendAddress ? null : (
                  <TYPE.main color={theme.red1}> ● Please input the address for sending coin.</TYPE.main>
                )}
              </AutoColumn>
            </AutoColumn>
          </TopWrapper>
